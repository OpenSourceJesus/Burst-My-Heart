...
/// </summary>
	void OnDrawGizmosSelected()
	{
		if(m_EnableDiagnostics && Application.isPlaying)
		{
			...
			Handles.Label(trs.position, "Ms: " + averageMilliseconds.ToString("0.000"));
		}
	}
#endif 
			...
			for(int yLoop = m_YRowStart; yLoop < m_YRowStart + m_GridHeight; yLoop++)
			{
				...
				for(int springLoop = 0; springLoop < m_VectorGridPoints[x, y].m_GridSpring.Length; springLoop++)
				{
					...
					if(spring.IsValid())
					{
						...
						{
							Debug.DrawLine(trs.TransformPoint(spring.m_Point1.m_Position), trs.TransformPoint(spring.m_Point2.m_OriginalPosition), Color.red);
						}
						else
						{
							Debug.DrawLine(trs.TransformPoint(spring.m_Point1.m_Position), trs.TransformPoint(spring.m_Point2.m_Position), Color.white);
						}
					}
				}
			}
	...
	public void AddGridForce(Vector3 worldPosition, float force, float radius, Color color, bool hasColor)
	{
		if(m_VectorGridPoints != null)
		{
			Vector3 localPosition = trs.InverseTransformPoint(worldPosition);
