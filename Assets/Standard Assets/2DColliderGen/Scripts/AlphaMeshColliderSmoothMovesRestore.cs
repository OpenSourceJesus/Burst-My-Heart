...
public class AlphaMeshColliderSmoothMovesRestore : MonoBehaviour {
	...
	public class RestoreData {
        ...
        public Vector2[] mEdgeColliderPoints = null;
#endif
    }
	//-------------------------------------------------------------------------
		...
		foreach (Transform child in node) {
			...
			if (alphaMeshColliderComponent != null) {
...
#endif
#if UNITY_SUPPORTS_EDGECOLLIDER2D
                EdgeCollider2D edgeCollider = alphaMeshColliderComponent.TargetNodeToAttachMeshCollider.gameObject.GetComponent<EdgeCollider2D>();
				if (edgeCollider != null) {
					...
					//	data.mPolygonColliderPaths[pathIndex] = polygonCollider.GetPath(pathIndex);
					//}
					data.mEdgeColliderPoints = edgeCollider.points;

					data.mIsTrigger = polygonCollider.isTrigger;
					// data.mConvex = meshCollider.convex; // Note: polygon colliders have no convex flag.
					data.mSharedMaterial2D = edgeCollider.sharedMaterial;
					//data.mSmoothSphereCollisions = meshCollider.smoothSphereCollisions; // Note: polygon colliders have no smoothSphereCollisions flag.
		...
		for (int index = 0; index < mDataToRestore.Length; ++index) {
...
#if UNITY_SUPPORTS_EDGECOLLIDER2D
            bool hasEdgeCollider = (data.mEdgeColliderPoints!= null);
#endif
            if (hasMeshCollider) {

                MeshCollider collider = restoreNode.GetComponent<MeshCollider>();
                if (collider == null) {
                    collider = restoreNode.gameObject.AddComponent<MeshCollider>(); // called at runtime only
                }
...
#if UNITY_SUPPORTS_EDGECOLLIDER2D
            else if (hasEdgeCollider) {
                EdgeCollider2D collider = restoreNode.GetComponent<EdgeCollider2D>();
                if (collider == null) {
                    collider = restoreNode.gameObject.AddComponent<EdgeCollider2D>();  // called at runtime only
                }
                
                collider.points = data.mEdgeColliderPoints;
                collider.isTrigger = data.mIsTrigger;
                ...
                if (collider == null) {
                    collider = restoreNode.gameObject.AddComponent<PolygonCollider2D>();  // called at runtime only
                }
			...
			if (hasSmoothMovesCollider) {
				...
				if (copyStateComponent == null) {
                    copyStateComponent = restoreNode.gameObject.AddComponent<AlphaMeshColliderCopyColliderEnabled>();  // called at runtime only
				}
			}
		}
	}
