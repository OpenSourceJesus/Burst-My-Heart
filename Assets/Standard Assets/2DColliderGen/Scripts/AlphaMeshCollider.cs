...
/// </summary>
[ExecuteInEditMode]
[AddComponentMenu("2D ColliderGen/Add AlphaMeshCollider")]
public class AlphaMeshCollider : MonoBehaviour {
    ...
    private const int REMOVED_FRAME_REFERENCE_INDEX_NOTREMOVED = -1;
    ...
    public static float CURRENT_2DCOLLIDERGEN_VERSION = 2.8f;
    ...
    public bool mHasUGUIImageSpriteComponent = false;
    protected UnityEngine.UI.Image mUGUIImageComponent = null;

    public bool mHasUGUIRawImageComponent = false;
    protected UnityEngine.UI.RawImage mUGUIRawImageComponent = null;
#endif
...
#endif
#if UNITY_SUPPORTS_EDGECOLLIDER2D
        [SerializeField] protected EdgeCollider2D[] mEdgeCollidersToSwitch = null;
#endif
        [SerializeField] protected MeshCollider[] mMeshCollidersToSwitch = null;
        [SerializeField] protected string[] mColliderIDStrings = new string[0];
        /// <summary>
        /// If a collider at a frame of an animation is removed, it references another frame's collider via an index. Set to -1 when the frame has not been removed.
        /// </summary>
        [SerializeField] protected int[] mRemovedFrameReferenceIndex = null;

        public void CopySharedParametersTo(AlphaMeshCollider targetAlphaMeshCollider) {

            AnimatedColliderParameters newParameters = new AnimatedColliderParameters();
            
            newParameters.mIsColliderAnimationUsed = this.mIsColliderAnimationUsed;

#if UNITY_4_3_AND_LATER
            newParameters.mSpriteFrames = this.mSpriteFrames;
            //PolygonCollider2D[] srcPolygonColliders = this.mPolygonCollidersToSwitch;
            //newParameters.mPolygonCollidersToSwitch = new PolygonCollider2D[srcPolygonColliders.Length];
#endif
#if UNITY_SUPPORTS_EDGECOLLIDER2D
            //EdgeCollider2D[] srcEdgeColliders = this.mEdgeCollidersToSwitch;
#endif
            //MeshCollider[] srcMeshColliders = this.mMeshCollidersToSwitch;

            newParameters.mColliderIDStrings = this.mColliderIDStrings;
            newParameters.mRemovedFrameReferenceIndex = this.mRemovedFrameReferenceIndex;

            targetAlphaMeshCollider.mAnimatedColliderParameters = newParameters;
        }
    ...
    // Setters and Getters
        ...
        public PolygonCollider2D[] PolygonCollidersToSwitch {
            ...
            set {
                ...
                if (mPolygonCollidersToSwitch != null) {
                    EnsureRemovedFrameReferenceIndexArrayIsSetup(mPolygonCollidersToSwitch.Length);
                }
            }
        }
#endif
#if UNITY_SUPPORTS_EDGECOLLIDER2D
        public EdgeCollider2D[] EdgeCollidersToSwitch {
            get {
            ...
            set {
                ...
                if (mEdgeCollidersToSwitch != null) {
                    EnsureRemovedFrameReferenceIndexArrayIsSetup(mEdgeCollidersToSwitch.Length);
                }
            }
        }
#endif
        public MeshCollider[] MeshCollidersToSwitch {
            ...
            set {
                ...
                if (mMeshCollidersToSwitch != null) {
                    EnsureRemovedFrameReferenceIndexArrayIsSetup(mMeshCollidersToSwitch.Length);
                }
            }
        }
        ...
        public int[] RemovedFrameReferenceIndex {
            get {
                int numColliderFrames = Mathf.Max(1, mColliderIDStrings.Length);
                if (mRemovedFrameReferenceIndex == null || mRemovedFrameReferenceIndex.Length != numColliderFrames) {
                    EnsureRemovedFrameReferenceIndexArrayIsSetup(numColliderFrames);
                }
                return mRemovedFrameReferenceIndex;
            }
            set {
        ...
        public int NumCollidersNeeded {
            get {
                int numNotRemovedColliders = Mathf.Max(1, mColliderIDStrings.Length);
                /*System.Array colliderArray = MeshCollidersToSwitch;
                if (MeshCollidersToSwitch != null) {
                    colliderArray = MeshCollidersToSwitch;
                }
#if UNITY_4_3_AND_LATER
                else if (PolygonCollidersToSwitch != null) {
                    colliderArray = PolygonCollidersToSwitch;
                }
#endif
#if UNITY_SUPPORTS_EDGECOLLIDER2D
                else if (EdgeCollidersToSwitch != null) {
                    colliderArray = EdgeCollidersToSwitch;
                }
#endif
                for (int index = 0; index < colliderArray.Length; ++index) {
                    if (colliderArray.GetValue(index) != null) {
                        ++numNotRemovedColliders;
                    }
                }*/

                if (mRemovedFrameReferenceIndex != null) {
                    for (int index = 0; index < mRemovedFrameReferenceIndex.Length; ++index) {
                        if (mRemovedFrameReferenceIndex[index] != REMOVED_FRAME_REFERENCE_INDEX_NOTREMOVED) {
                            --numNotRemovedColliders;
                        }
                    }
                }

                if (numNotRemovedColliders <= 0) {
                    numNotRemovedColliders = 1; // we need at least one!
                }
                return numNotRemovedColliders;
            }
        }

        public bool IsFrameRemoved(int frameIndex) {
            if (mRemovedFrameReferenceIndex == null || frameIndex >= mRemovedFrameReferenceIndex.Length) {
                return false;
            }
            return mRemovedFrameReferenceIndex[frameIndex] != REMOVED_FRAME_REFERENCE_INDEX_NOTREMOVED;
        }

        public void EnsureRemovedFrameReferenceIndexArrayIsSetup(int numFrames) {

            if (mRemovedFrameReferenceIndex == null || mRemovedFrameReferenceIndex.Length != numFrames) {
                mRemovedFrameReferenceIndex = new int[numFrames];
                for (int index = 0; index < numFrames; ++index) {
                    mRemovedFrameReferenceIndex[index] = REMOVED_FRAME_REFERENCE_INDEX_NOTREMOVED;
                }
            }
        }
    }
    ...
    public class RegionIndependentParameters : RegionIndependentParametersBase {
        ...
        [SerializeField, FormerlySerializedAs("mNumCollidersNeeded")] protected int mNumColliderAnimationFrames = 1;
#if UNITY_4_3_AND_LATER
        ...
        public int NumColliderAnimationFrames {
            get {
                return mNumColliderAnimationFrames;
            }
            set {
                mNumColliderAnimationFrames = value;
            }
        }
    ...
    [SerializeField] protected float mVersion = CURRENT_2DCOLLIDERGEN_VERSION;

    // Setters and Getters
    public MeshCollider TargetMeshColliderComponentNoReferencing {
        get {
            if (mAnimatedColliderParameters.MeshCollidersToSwitch != null && mActiveColliderIndex < mAnimatedColliderParameters.MeshCollidersToSwitch.Length) {
                MeshCollider collider = mAnimatedColliderParameters.MeshCollidersToSwitch[mActiveColliderIndex];
                return collider;
            }
            else {
                return TargetNodeToAttachMeshCollider.GetComponent<MeshCollider>();
            }
        }
    }
#if UNITY_4_3_AND_LATER
    public PolygonCollider2D TargetPolygonCollider2DComponentNoReferencing {
        get {
            if (mAnimatedColliderParameters.PolygonCollidersToSwitch != null && mActiveColliderIndex < mAnimatedColliderParameters.PolygonCollidersToSwitch.Length) {
                PolygonCollider2D collider = mAnimatedColliderParameters.PolygonCollidersToSwitch[mActiveColliderIndex];
                return collider;
            }
            else {
                return TargetNodeToAttachMeshCollider.GetComponent<PolygonCollider2D>();
            }
        }
    }
#endif
#if UNITY_SUPPORTS_EDGECOLLIDER2D
    public EdgeCollider2D TargetEdgeCollider2DComponentNoReferencing {
        get {
            if (mAnimatedColliderParameters.EdgeCollidersToSwitch != null && mActiveColliderIndex < mAnimatedColliderParameters.EdgeCollidersToSwitch.Length) {
                return mAnimatedColliderParameters.EdgeCollidersToSwitch[mActiveColliderIndex];
            }
            else {
                return TargetNodeToAttachMeshCollider.GetComponent<EdgeCollider2D>();
            }
        }
    }
#endif
    ...
    public int NumColliderAnimationFrames {
        get { return mRegionIndependentParameters.NumColliderAnimationFrames; }
    }

    public bool HasMultipleColliderFrames {
        get {
            return (mRegionIndependentParameters.NumColliderAnimationFrames > 1);
        }
    }
    ...
    public AnimatedColliderParameters AnimatedColliderParams {
        get {
            return mAnimatedColliderParameters;
        }
    }
	...
	public Mesh[] ColliderMeshes {
		...
		set {
			if (mRegionIndependentParameters.TargetColliderType == TargetColliderType.MeshCollider) {
                ...
                int numAnimationFrames = this.NumColliderAnimationFrames;
                MeshCollider[] colliders = EnsureHasMeshColliderComponentsAndReferences(numAnimationFrames, value.Length);
		...
		set {
			...
			if (value != oldApplySmoothMovesScaleAnim) {
				if (this.HasSmoothMovesBoneAnimationParent) {
					...
					if (oldMeshCollider) {
                        UndoAware.DestroyImmediate(oldMeshCollider);
					}
				}
			}
    ...
    //-------------------------------------------------------------------------
    public void AssignAnimatedColliderParamsFromOther(AnimatedColliderParameters otherParameters) {

        otherParameters.CopySharedParametersTo(this);
        
        // Note: now we don't deep copy the colliders, only the parameters.
        /*int numColliders = otherParameters.NumCollidersNeeded;
        int numFrames = 0;
#if UNITY_4_3_AND_LATER
        PolygonCollider2D[] srcPolygonColliders = otherParameters.PolygonCollidersToSwitch;
        if (srcPolygonColliders != null) {
            numFrames = otherParameters.PolygonCollidersToSwitch.Length;
            EnsureHasPolygonCollider2DComponentsAndReferences(numFrames, numColliders);

            PolygonCollider2D[] dstPolygonColliders = mAnimatedColliderParameters.PolygonCollidersToSwitch;
            for (int colliderIndex = 0; colliderIndex < dstPolygonColliders.Length; ++colliderIndex) {

                if (srcPolygonColliders[colliderIndex] == null) {
                    dstPolygonColliders[colliderIndex] = null;
                }
                else {
                    dstPolygonColliders[colliderIndex].pathCount = srcPolygonColliders[colliderIndex].pathCount;
                    for (int pathIndex = 0; pathIndex < dstPolygonColliders[colliderIndex].pathCount; ++pathIndex) {
                        dstPolygonColliders[colliderIndex].SetPath(pathIndex, dstPolygonColliders[colliderIndex].GetPath(pathIndex));
                    }
                }
            }
        }
...
#endif
#if UNITY_SUPPORTS_EDGECOLLIDER2D
        EdgeCollider2D[] srcEdgeColliders = otherParameters.EdgeCollidersToSwitch;
        if (srcEdgeColliders != null) {
            numFrames = otherParameters.EdgeCollidersToSwitch.Length;
            EnsureHasEdgeCollider2DComponentsAndReferences(numFrames, numColliders);

            EdgeCollider2D[] dstEdgeColliders = mAnimatedColliderParameters.EdgeCollidersToSwitch;
            for (int colliderIndex = 0; colliderIndex < dstEdgeColliders.Length; ++colliderIndex) {
                if (srcEdgeColliders[colliderIndex] == null) {
                    dstEdgeColliders[colliderIndex] = null;
                }
                else {
                   dstEdgeColliders[colliderIndex].points = srcEdgeColliders[colliderIndex].points;
                }
            }
        }
        else {
            mAnimatedColliderParameters.EdgeCollidersToSwitch = null;
        }
#endif
        MeshCollider[] srcMeshColliders = otherParameters.MeshCollidersToSwitch;
        if (srcMeshColliders != null) {
            numFrames = otherParameters.MeshCollidersToSwitch.Length;
            EnsureHasMeshColliderComponentsAndReferences(numFrames, numColliders);

            MeshCollider[] dstMeshColliders = mAnimatedColliderParameters.MeshCollidersToSwitch;
            for (int colliderIndex = 0; colliderIndex < dstMeshColliders.Length; ++colliderIndex) {
                if (srcMeshColliders[colliderIndex] == null) {
                    dstMeshColliders[colliderIndex] = null;
                }
                else {
                    dstMeshColliders[colliderIndex].sharedMesh = null;
                    dstMeshColliders[colliderIndex].sharedMesh = srcMeshColliders[colliderIndex].sharedMesh;
                }
            }
        }
        else {
    ...
    //-------------------------------------------------------------------------
    public void EnableIslands(int startIndex, int endIndex) {
		
		int maxIndex = Mathf.Min(endIndex, mIslandRegionParameters.Length);
		for (int index = startIndex; index < maxIndex; ++index) {
			mIslandRegionParameters[index].EnableRegion = true;
		}
	}
	
	//-------------------------------------------------------------------------
	public void DisableIslands(int startIndex, int endIndex) {
		int maxIndex = Mathf.Min(endIndex, mIslandRegionParameters.Length);
		for (int index = startIndex; index < maxIndex; ++index) {
			mIslandRegionParameters[index].EnableRegion = false;
		}
	}
	
	//-------------------------------------------------------------------------
	public void EnableSeaRegions(int startIndex, int endIndex) {
		
		int maxIndex = Mathf.Min(endIndex, mSeaRegionParameters.Length);
		for (int index = startIndex; index < maxIndex; ++index) {
			mSeaRegionParameters[index].EnableRegion = true;
		}
	}
	
	//-------------------------------------------------------------------------
	public void DisableSeaRegions(int startIndex, int endIndex) {
		int maxIndex = Mathf.Min(endIndex, mSeaRegionParameters.Length);
		for (int index = startIndex; index < maxIndex; ++index) {
			mSeaRegionParameters[index].EnableRegion = false;
		}
	}
	...
	public void UpdateAlphaMeshCollider() {
        ...
        bool isInitializedThisFrame = !mWasInitialized;
		if (!mWasInitialized)
        ...
        CheckForUGUIImageSpriteComponent(out mHasUGUIImageSpriteComponent, out mUGUIImageComponent);

        CheckForUGUIRawImageComponent(out mHasUGUIRawImageComponent, out mUGUIRawImageComponent);
#endif
		...
		if (mInactiveBaseImageWidth == 0 || UsedTexture == null) {
			InitTextureParams();
		}
        ...
        bool isMeshColliderMissing = false;
        if (mRegionIndependentParameters.TargetColliderType == TargetColliderType.MeshCollider) {
            MeshCollider meshCollider = TargetNodeToAttachMeshCollider.GetComponent<MeshCollider>();
            isMeshColliderMissing = (meshCollider == null || meshCollider.sharedMesh == null);
        }
        ...
        if (mRegionIndependentParameters.TargetColliderType == TargetColliderType.PolygonCollider2D) {
            ...
            isPolygonColliderMissing = (polygonCollider == null);
        }
#endif
        bool isEdgeColliderMissing = false;
#if UNITY_SUPPORTS_EDGECOLLIDER2D
        if (mRegionIndependentParameters.TargetColliderType == TargetColliderType.EdgeCollider2D) {
            EdgeCollider2D edgeCollider = TargetNodeToAttachMeshCollider.GetComponent<EdgeCollider2D>();
            isEdgeColliderMissing = (edgeCollider == null);
        }   
#endif

        if (isMeshColliderMissing || isPolygonColliderMissing || isEdgeColliderMissing || hasChangedFlipState) {
			
			AlphaMeshColliderRegistry.Instance.ReloadOrRecalculateSingleCollider(this);
		}
        if (isInitializedThisFrame && mHasUnity43SpriteAnimatorComponent) {
            AlphaMeshColliderRegistry.Instance.ReloadOrRecalculateAllFramesAtColliderAndUpdateSimilar(this);
        }
	}
    
    protected void EnsureRemovedFrameReferenceIndexArrayIsSetup() {
#if UNITY_4_3_AND_LATER
        mAnimatedColliderParameters.EnsureRemovedFrameReferenceIndexArrayIsSetup(this.NumColliderAnimationFrames);
#endif
    }

    public void ResetRemovedFrameReferenceIndexArray() {
#if UNITY_4_3_AND_LATER
        this.mAnimatedColliderParameters.RemovedFrameReferenceIndex = null;
        EnsureRemovedFrameReferenceIndexArrayIsSetup(); // reset it completely to non-referenced values.
#endif
    }

#if UNITY_4_3_AND_LATER
    //-------------------------------------------------------------------------
    protected PolygonCollider2D[] EnsureHasPolygonCollider2DComponentsAndReferences(int numFrames, int numComponentsRequired) {

		PolygonCollider2D[] attachedColliders = TargetNodeToAttachMeshCollider.GetComponents<PolygonCollider2D>();
		if (attachedColliders == null || attachedColliders.Length == 0) {
#if UNITY_SUPPORTS_EDGECOLLIDER2D
            RemoveExistingEdgeCollider2DComponents();
#endif
            RemoveExistingMeshColliderComponents(); // unfortunately we can't have multiple collider types, it would produce an error.
            SwitchRigidbodyToRigidbody2D();
			attachedColliders = AddEmptyPolygonCollider2DComponents(numComponentsRequired);
		}
		else if (attachedColliders.Length != numComponentsRequired) {
			if (attachedColliders.Length > numComponentsRequired) {
				RemoveExistingPolygonCollider2DComponents();
				attachedColliders = AddEmptyPolygonCollider2DComponents(numComponentsRequired);
			}
			else {
				AddEmptyPolygonCollider2DComponents(numComponentsRequired - attachedColliders.Length);
				attachedColliders = TargetNodeToAttachMeshCollider.GetComponents<PolygonCollider2D>();
			}
		}

        PolygonCollider2D[] colliderFrameReferences = mAnimatedColliderParameters.PolygonCollidersToSwitch;
        if (colliderFrameReferences == null || colliderFrameReferences.Length != numFrames) {
            colliderFrameReferences = new PolygonCollider2D[numFrames];
        }
        AssignCollidersToColliderFrameReferenceArray(colliderFrameReferences, attachedColliders);
        
        mAnimatedColliderParameters.PolygonCollidersToSwitch = colliderFrameReferences;
		return attachedColliders;
	}
#endif

    //-------------------------------------------------------------------------
    protected void AssignCollidersToColliderFrameReferenceArray(System.Array colliderFrameReferences, System.Array attachedColliders) {

        int attachedColliderIndex = 0;
        
        for (int frameIndex = 0; frameIndex < colliderFrameReferences.Length; ++frameIndex) {
            if (mAnimatedColliderParameters.IsFrameRemoved(frameIndex)) {
                colliderFrameReferences.SetValue(null, frameIndex);
            }
            else {
                colliderFrameReferences.SetValue(attachedColliders.GetValue(attachedColliderIndex++), frameIndex);
            }
        }
    }
...
#if UNITY_SUPPORTS_EDGECOLLIDER2D
    //-------------------------------------------------------------------------
    protected EdgeCollider2D[] EnsureHasEdgeCollider2DComponentsAndReferences(int numFrames, int numComponentsRequired) {

        int numIslandCollidersNeeded = GetNumActiveIslandsAtCurrentFrame();
        if (numIslandCollidersNeeded > 0) {
            // otherwise generated collider data is not initialized yet.
            numComponentsRequired *= numIslandCollidersNeeded;
        }

        EdgeCollider2D[] attachedColliders = TargetNodeToAttachMeshCollider.GetComponents<EdgeCollider2D>();
        if (attachedColliders == null || attachedColliders.Length == 0) {
            RemoveExistingMeshColliderComponents(); // unfortunately we can't have multiple collider types, it would produce an error.
            RemoveExistingPolygonCollider2DComponents(); // same here.
            SwitchRigidbodyToRigidbody2D();
            attachedColliders = AddEmptyEdgeCollider2DComponents(numComponentsRequired);
        }
        else if (attachedColliders.Length != numComponentsRequired) {
            if (attachedColliders.Length > numComponentsRequired) {
                RemoveExistingEdgeCollider2DComponents();
                ...
                AddEmptyEdgeCollider2DComponents(numComponentsRequired - attachedColliders.Length);
                attachedColliders = TargetNodeToAttachMeshCollider.GetComponents<EdgeCollider2D>();
            }
        }

        EdgeCollider2D[] colliderFrameReferences = mAnimatedColliderParameters.EdgeCollidersToSwitch;
        if (colliderFrameReferences == null || colliderFrameReferences.Length != numFrames) {
            colliderFrameReferences = new EdgeCollider2D[numFrames];
        }
        AssignCollidersToColliderFrameReferenceArray(colliderFrameReferences, attachedColliders);
        
        mAnimatedColliderParameters.EdgeCollidersToSwitch = colliderFrameReferences;
        return attachedColliders;
    }
#endif

    //-------------------------------------------------------------------------
    protected MeshCollider[] EnsureHasMeshColliderComponentsAndReferences(int numFrames, int numComponentsRequired) {
		MeshCollider[] attachedColliders = TargetNodeToAttachMeshCollider.GetComponents<MeshCollider>();
		if (attachedColliders == null || attachedColliders.Length == 0) {
#if UNITY_SUPPORTS_EDGECOLLIDER2D
            RemoveExistingEdgeCollider2DComponents();
#endif
#if UNITY_4_3_AND_LATER
            RemoveExistingPolygonCollider2DComponents(); // unfortunately we can't have multiple collider types, it would produce an error.
            SwitchRigidbody2DToRigidbody();
#endif
            attachedColliders = AddEmptyMeshColliderComponents(numComponentsRequired);
		}
		else if (attachedColliders.Length != numComponentsRequired) {
			if (attachedColliders.Length > numComponentsRequired) {
				...
				AddEmptyMeshColliderComponents(numComponentsRequired - attachedColliders.Length);
				attachedColliders = TargetNodeToAttachMeshCollider.GetComponents<MeshCollider>();
			}
		}

        MeshCollider[] colliderFrameReferences = mAnimatedColliderParameters.MeshCollidersToSwitch;
        if (colliderFrameReferences == null || colliderFrameReferences.Length != numFrames) {
            colliderFrameReferences = new MeshCollider[numFrames];
        }
        AssignCollidersToColliderFrameReferenceArray(colliderFrameReferences, attachedColliders);

        mAnimatedColliderParameters.MeshCollidersToSwitch = colliderFrameReferences;
        return attachedColliders;
	}
	...
	protected bool SwitchRigidbody2DToRigidbody() {
        ...
        UndoAware.DestroyImmediate(rigidbody2D);
        Rigidbody newRigidbody = UndoAware.AddComponent<Rigidbody>(TargetNodeToAttachMeshCollider.gameObject);
	...
	protected bool SwitchRigidbodyToRigidbody2D() {
        ...
        UndoAware.DestroyImmediate(rigidbody);
        Rigidbody2D newRigidbody = UndoAware.AddComponent<Rigidbody2D>(TargetNodeToAttachMeshCollider.gameObject);
		newRigidbody.mass = mass;
		newRigidbody.drag = drag;
		newRigidbody.angularDrag = angularDrag;
	...
	protected PolygonCollider2D[] AddEmptyPolygonCollider2DComponents(int numComponentsToAdd) {
		...
		for (int count = 0; count < numComponentsToAdd; ++count) {
            PolygonCollider2D newCollider = UndoAware.AddComponent<PolygonCollider2D>(targetNode.gameObject);
            newCollider.isTrigger = mRegionIndependentParameters.IsTrigger;
...
#if UNITY_SUPPORTS_EDGECOLLIDER2D
    //-------------------------------------------------------------------------
    protected EdgeCollider2D[] AddEmptyEdgeCollider2DComponents(int numComponentsToAdd) {
        ...
        List<EdgeCollider2D> resultColliders = new List<EdgeCollider2D>();
        for (int count = 0; count < numComponentsToAdd; ++count) {
            EdgeCollider2D newCollider = UndoAware.AddComponent<EdgeCollider2D>(targetNode.gameObject);
            newCollider.isTrigger = mRegionIndependentParameters.IsTrigger;
            resultColliders.Add(newCollider);
        }
    ...
    protected MeshCollider[] AddEmptyMeshColliderComponents(int numComponentsToAdd) {
		...
		for (int count = 0; count < numComponentsToAdd; ++count) {
            MeshCollider newCollider = UndoAware.AddComponent<MeshCollider>(targetNode.gameObject);
            if (this.mRegionIndependentParameters.Convex) {
                newCollider.convex = this.mRegionIndependentParameters.Convex;
            }
            if (this.mRegionIndependentParameters.IsTrigger) {
                newCollider.isTrigger = this.mRegionIndependentParameters.IsTrigger;
            }
            resultColliders.Add(newCollider);
		}
	...
	protected void InitWithPreferencesValues() {
        ...
        this.mRegionIndependentParameters.AlphaOpaqueThreshold = AlphaMeshColliderPreferences.Instance.DefaultAlphaOpaqueThreshold;
        ...
        this.mRegionIndependentParameters.IsTrigger = AlphaMeshColliderPreferences.Instance.DefaultTrigger;
        this.mRegionIndependentParameters.FlipInsideOutside = AlphaMeshColliderPreferences.Instance.DefaultFlipNormals;
        this.mRegionIndependentParameters.LowerTop = AlphaMeshColliderPreferences.Instance.DefaultLowerTop;
        this.mRegionIndependentParameters.RaiseBottom = AlphaMeshColliderPreferences.Instance.DefaultRaiseBottom;
        this.mRegionIndependentParameters.CutLeft = AlphaMeshColliderPreferences.Instance.DefaultCutLeft;
        this.mRegionIndependentParameters.CutRight = AlphaMeshColliderPreferences.Instance.DefaultCutRight;
        this.mRegionIndependentParameters.ExpandTop = AlphaMeshColliderPreferences.Instance.DefaultExpandTop;
        this.mRegionIndependentParameters.ExpandBottom = AlphaMeshColliderPreferences.Instance.DefaultExpandBottom;
        this.mRegionIndependentParameters.ExpandLeft = AlphaMeshColliderPreferences.Instance.DefaultExpandLeft;
        this.mRegionIndependentParameters.ExpandRight = AlphaMeshColliderPreferences.Instance.DefaultExpandRight;
        mWasInitialized = true;
	}

    //-------------------------------------------------------------------------
    public void UpdateColliderComponentValues() {

        bool isTrigger = mRegionIndependentParameters.IsTrigger;
        bool isConvex = mRegionIndependentParameters.Convex;
        Transform targetNode = TargetNodeToAttachMeshCollider;

        MeshCollider[] meshColliders = targetNode.GetComponents<MeshCollider>();
        for (int index = 0; index < meshColliders.Length; ++index) {
            meshColliders[index].isTrigger = isTrigger;
            meshColliders[index].convex = isConvex;
        }

        Collider2D[] polygonOrEdgeColliders = targetNode.GetComponents<Collider2D>();
        for (int index = 0; index < polygonOrEdgeColliders.Length; ++index) {
            polygonOrEdgeColliders[index].isTrigger = isTrigger;
        }
    }

    //-------------------------------------------------------------------------
    public void ChangeSyncToParentSpriteRenderer(bool doSyncToParent) {
        mRegionIndependentParameters.SyncToParentSpriteRenderer = doSyncToParent;

        UpdateSyncToParentSpriteRenderer();
    }

    //-------------------------------------------------------------------------
    public void UpdateSyncToParentSpriteRenderer() {
        RuntimeAnimatedColliderSwitch switchComponent = this.GetComponent<RuntimeAnimatedColliderSwitch>();
        if (switchComponent) {
            switchComponent.SyncToParentSpriteRenderer = mRegionIndependentParameters.SyncToParentSpriteRenderer;
        }
    }
	...
	public void IncreaseActiveColliderFrameIndex() {
		int numFrames = this.NumColliderAnimationFrames;
		int targetIndex = mActiveColliderIndex + 1;
    ...
    //-------------------------------------------------------------------------
    public bool IsCurrentFrameRemoved() {
        return mAnimatedColliderParameters.IsFrameRemoved(mActiveColliderIndex);
    }

    //-------------------------------------------------------------------------
    public int CurrentRemovedFrameDirectReferenceIndex() {

        return mAnimatedColliderParameters.RemovedFrameReferenceIndex[mActiveColliderIndex];
    }
    
    //-------------------------------------------------------------------------
    public void RemoveActiveColliderFrame() {
#if UNITY_4_3_AND_LATER
        int targetFrameIndex = mActiveColliderIndex;
        int numFrames = this.NumColliderAnimationFrames;
        int replacementFrameIndex = (targetFrameIndex + numFrames - 1) % numFrames;

        EnsureRemovedFrameReferenceIndexArrayIsSetup();
        mAnimatedColliderParameters.RemovedFrameReferenceIndex[targetFrameIndex] = replacementFrameIndex;

        if (mAnimatedColliderParameters.PolygonCollidersToSwitch != null) {
            if (mAnimatedColliderParameters.PolygonCollidersToSwitch[targetFrameIndex] != null) {
                UndoAware.DestroyImmediate(mAnimatedColliderParameters.PolygonCollidersToSwitch[targetFrameIndex]);
            }
            mAnimatedColliderParameters.PolygonCollidersToSwitch[targetFrameIndex] = null;
        }
        if (mAnimatedColliderParameters.MeshCollidersToSwitch != null) {
            if (mAnimatedColliderParameters.MeshCollidersToSwitch[targetFrameIndex] != null) {
                UndoAware.DestroyImmediate(mAnimatedColliderParameters.MeshCollidersToSwitch[targetFrameIndex]);
            }
            mAnimatedColliderParameters.MeshCollidersToSwitch[targetFrameIndex] = null;
        }
    ...
    //-------------------------------------------------------------------------
    public void AddActiveColliderFrameBackIn() {
#if UNITY_4_3_AND_LATER
        if (mAnimatedColliderParameters.PolygonCollidersToSwitch != null && mAnimatedColliderParameters.PolygonCollidersToSwitch.Length > 0) {
            AddAlreadyGenerated2DPolygonColliderBackIn();
        }
        if (mAnimatedColliderParameters.MeshCollidersToSwitch != null && mAnimatedColliderParameters.MeshCollidersToSwitch.Length > 0) {
            AddAlreadyGeneratedMeshColliderBackIn();
        }
    ...
    //-------------------------------------------------------------------------
    public void DecreaseActiveFrameReferenceFrameIndex() {
#if UNITY_4_3_AND_LATER
        int srcFrameIndex = mActiveColliderIndex;
        int numFrames = this.NumColliderAnimationFrames;
        int currentReplacementFrameIndex = mAnimatedColliderParameters.RemovedFrameReferenceIndex[srcFrameIndex];
        int replacementFrameIndex = (currentReplacementFrameIndex + numFrames - 1) % numFrames;
        if (replacementFrameIndex == srcFrameIndex) { // we cannot reference our own frame since it's deleted.
            replacementFrameIndex = (replacementFrameIndex + numFrames - 1) % numFrames;
        }

        EnsureRemovedFrameReferenceIndexArrayIsSetup();
        mAnimatedColliderParameters.RemovedFrameReferenceIndex[srcFrameIndex] = replacementFrameIndex;
    ...
    //-------------------------------------------------------------------------
    public void IncreaseActiveFrameReferenceFrameIndex() {
#if UNITY_4_3_AND_LATER
        int srcFrameIndex = mActiveColliderIndex;
        int numFrames = this.NumColliderAnimationFrames;
        int currentReplacementFrameIndex = mAnimatedColliderParameters.RemovedFrameReferenceIndex[srcFrameIndex];
        int replacementFrameIndex = (currentReplacementFrameIndex + 1) % numFrames;
        if (replacementFrameIndex == srcFrameIndex) { // we cannot reference our own frame since it's deleted.
            replacementFrameIndex = (replacementFrameIndex + 1) % numFrames;
        }
    ...
    //-------------------------------------------------------------------------
    public void SetActiveFrameReferenceFrameIndex(int referenceFrameIndex) {
#if UNITY_4_3_AND_LATER
        int targetFrameIndex = mActiveColliderIndex;
        int numFrames = this.NumColliderAnimationFrames;
        int replacementFrameIndex = referenceFrameIndex;
        if (replacementFrameIndex == mActiveColliderIndex) {
            replacementFrameIndex = (replacementFrameIndex + numFrames - 1) % numFrames;
        }
    ...
    public void EnableAllColliderFrames() {
		...
		if (mAnimatedColliderParameters.PolygonCollidersToSwitch != null) {
			for (int index = 0; index < mAnimatedColliderParameters.PolygonCollidersToSwitch.Length; ++index) {
                ...
                PolygonCollider2D collider = mAnimatedColliderParameters.PolygonCollidersToSwitch[index];
                if (collider != null) {
		...
		if (mAnimatedColliderParameters.MeshCollidersToSwitch != null) {
			for (int index = 0; index < mAnimatedColliderParameters.MeshCollidersToSwitch.Length; ++index) {
                MeshCollider collider = mAnimatedColliderParameters.MeshCollidersToSwitch[index];
                if (collider != null) {
                    collider.enabled = true;
                }
            }
		...
		if (mAnimatedColliderParameters.PolygonCollidersToSwitch != null && mAnimatedColliderParameters.PolygonCollidersToSwitch.Length > 0) {
			for (int index = 0; index < mAnimatedColliderParameters.PolygonCollidersToSwitch.Length; ++index) {
				if (index != mActiveColliderIndex) {
                    if (mAnimatedColliderParameters.PolygonCollidersToSwitch[index] != null) {
                        mAnimatedColliderParameters.PolygonCollidersToSwitch[index].enabled = false;
                    }
				}
			}
            PolygonCollider2D colliderToEnable = PolygonCollidersToSwitch(mActiveColliderIndex);
            colliderToEnable.enabled = true;
        }
		if (mAnimatedColliderParameters.MeshCollidersToSwitch != null && mAnimatedColliderParameters.MeshCollidersToSwitch.Length > 0) {
			for (int index = 0; index < mAnimatedColliderParameters.MeshCollidersToSwitch.Length; ++index) {
				if (index != mActiveColliderIndex) {
                    if (mAnimatedColliderParameters.MeshCollidersToSwitch[index]) {
                        mAnimatedColliderParameters.MeshCollidersToSwitch[index].enabled = false;
                    }
				}
			}
            MeshCollider colliderToEnable = MeshCollidersToSwitch(mActiveColliderIndex);
            colliderToEnable.enabled = true;
        }
#endif
	}
...
#if UNITY_4_3_AND_LATER
    public PolygonCollider2D PolygonCollidersToSwitch(int frameIndex) {
        int unusedFinallyReferencedFrameIndex = 0;
        return PolygonCollidersToSwitch(frameIndex, out unusedFinallyReferencedFrameIndex);
    }

    public PolygonCollider2D PolygonCollidersToSwitch(int frameIndex, out int finallyReferencedFrameIndex) {
        PolygonCollider2D collider = mAnimatedColliderParameters.PolygonCollidersToSwitch[frameIndex];
        if (collider == null) {
            int replacementIndex = mAnimatedColliderParameters.RemovedFrameReferenceIndex[frameIndex];
            return PolygonCollidersToSwitch(replacementIndex, out finallyReferencedFrameIndex);
        }
        else {
            finallyReferencedFrameIndex = frameIndex;
            return collider;
        }
    }

    public MeshCollider MeshCollidersToSwitch(int frameIndex) {
        int unusedFinallyReferencedFrameIndex = 0;
        return MeshCollidersToSwitch(frameIndex, out unusedFinallyReferencedFrameIndex);
    }

    public MeshCollider MeshCollidersToSwitch(int frameIndex, out int finallyReferencedFrameIndex) {
        MeshCollider collider = mAnimatedColliderParameters.MeshCollidersToSwitch[frameIndex];
        if (collider == null) {
            int replacementIndex = mAnimatedColliderParameters.RemovedFrameReferenceIndex[frameIndex];
            return MeshCollidersToSwitch(replacementIndex, out finallyReferencedFrameIndex);
        }
        else {
            finallyReferencedFrameIndex = frameIndex;
            return collider;
	...
	public void RecalculateAllColliderFrames() {
        ...
        ResetRemovedFrameReferenceIndexArray();
	...
	//-------------------------------------------------------------------------
    public void ShowOnlyCurrentFrame() {
        EnableOnlyActiveColliderFrame();
    }
...
#if UNITY_SUPPORTS_EDGECOLLIDER2D
        else if ((mRegionIndependentParameters.TargetColliderType == TargetColliderType.EdgeCollider2D)) {
            if (!SetEmpty2DEdgeCollider()) {
                wereAllSuccessful = false;
            }
        }
#endif
	...
	bool LoadAlreadyGeneratedCollider() {
		...
		if ((mRegionIndependentParameters.TargetColliderType == TargetColliderType.PolygonCollider2D)) {
            ...
            if (!LoadAlreadyGeneratedColliderTo2DEdgeCollider()) {
                wereAllSuccessful = false;
            }
        }
#endif
	...
	bool SetEmpty2DPolygonCollider() {
		PolygonCollider2D collider = this.TargetPolygonCollider2DComponentNoReferencing;
    ...
    //-------------------------------------------------------------------------
    bool AddAlreadyGenerated2DPolygonColliderBackIn() {

        PolygonCollider2D collider = this.TargetPolygonCollider2DComponentNoReferencing;
        if (collider == null) {
            PolygonCollider2D[] addedColliderArray = AddEmptyPolygonCollider2DComponents(1);
            collider = addedColliderArray[0];

            mAnimatedColliderParameters.PolygonCollidersToSwitch[mActiveColliderIndex] = collider;
            PolygonCollider2D[] attachedColliders = TargetNodeToAttachMeshCollider.GetComponents<PolygonCollider2D>();
            CorrectColliderOrder(attachedColliders, mAnimatedColliderParameters.PolygonCollidersToSwitch);

            mAnimatedColliderParameters.RemovedFrameReferenceIndex[mActiveColliderIndex] = REMOVED_FRAME_REFERENCE_INDEX_NOTREMOVED;

            LoadAlreadyGeneratedColliderTo2DPolygonCollider();
        }
        return true;
    }
    ...
    bool LoadAlreadyGeneratedColliderTo2DPolygonCollider() {
        ...
        int finallyReferencedFrameIndex = mActiveColliderIndex;
        PolygonCollider2D collider = this.PolygonCollidersToSwitch(mActiveColliderIndex, out finallyReferencedFrameIndex); // with referencing;

        int previouslyActiveFrame = mActiveColliderIndex;
        SetActiveColliderFrameIndex(finallyReferencedFrameIndex);

        int numActiveIslands = 0;
		ColliderRegionData[] colliderRegions = mGeneratedColliderData[finallyReferencedFrameIndex].mColliderRegions;
        ...
        SetActiveColliderFrameIndex(previouslyActiveFrame);
        return true;
	}

    //-------------------------------------------------------------------------
    void CorrectColliderOrder(System.Array attachedColliderComponents, System.Array collidersInDesiredOrder) {

        ArrayList attachedColliderComponentsAsList = new ArrayList(attachedColliderComponents);

        int desiredColliderLocationIndex = 0;
        for (int frameIndex = 0; frameIndex < collidersInDesiredOrder.Length; ++frameIndex) {
            Component desiredCollider = collidersInDesiredOrder.GetValue(frameIndex) as Component;
            if (desiredCollider == null) {
                continue;
            }
            int currentLocationIndexOfDesiredCollider = attachedColliderComponentsAsList.IndexOf(desiredCollider);
            int indexDifference = currentLocationIndexOfDesiredCollider - desiredColliderLocationIndex;

            if (indexDifference != 0) {
                
                attachedColliderComponentsAsList.RemoveAt(currentLocationIndexOfDesiredCollider);
                attachedColliderComponentsAsList.Insert(desiredColliderLocationIndex, desiredCollider);

                if (indexDifference > 0) {
                    while (indexDifference-- > 0) { // is too far down
                        UnityEditorInternal.ComponentUtility.MoveComponentUp(desiredCollider);
                    }
                }
                else {
                    while (indexDifference++ < 0) { // is too far up
                        UnityEditorInternal.ComponentUtility.MoveComponentDown(desiredCollider);
                    }
                }
            }

            ++desiredColliderLocationIndex;
        }
    }
...
#if UNITY_SUPPORTS_EDGECOLLIDER2D
    //-------------------------------------------------------------------------
    bool SetEmpty2DEdgeCollider() {
        EdgeCollider2D collider = this.TargetEdgeCollider2DComponentNoReferencing;
    ...
    //-------------------------------------------------------------------------
    bool LoadAlreadyGeneratedColliderTo2DEdgeCollider() {

        //EdgeCollider2D collider = this.TargetEdgeCollider2DComponent;
        ...
        int activeIslandIndex = 0;
        //List<Vector2> resultEdgePoints = new List<Vector2>();

        EdgeCollider2D[] colliders = TargetNodeToAttachMeshCollider.GetComponents<EdgeCollider2D>(); // FIXXME: clean this code up.

        for (int regionIndex = 0; regionIndex < colliderRegions.Length; ++regionIndex) {
            if (colliderRegions[regionIndex].mReducedOutlineVertices != null && colliderRegions[regionIndex].mReducedOutlineVertices.Count != 0) {

                Vector2[] transformedPolygon = new Vector2[colliderRegions[regionIndex].mReducedOutlineVertices.Count + 1]; // + 1 for the looped end vertex
                TransformReducedOutline(colliderRegions[regionIndex].mReducedOutlineVertices, transformedPolygon);
                transformedPolygon[transformedPolygon.Length - 1] = transformedPolygon[0];
                colliders[activeIslandIndex].points = transformedPolygon;
                ++activeIslandIndex;
            }
        }
        //collider.points = resultEdgePoints.ToArray();
        return true;
    }
#endif

    //-------------------------------------------------------------------------
    int GetNumActiveIslandsAtCurrentFrame() {
        return GetNumActiveIslandsAtFrame(mActiveColliderIndex);
    }

    //-------------------------------------------------------------------------
    int GetNumActiveIslandsAtFrame(int frameIndex) {
        int numActiveIslands = 0;

        if (mGeneratedColliderData == null || mGeneratedColliderData[frameIndex].mColliderRegions == null) {
            return 0;
        }

        ColliderRegionData[] colliderRegions = mGeneratedColliderData[frameIndex].mColliderRegions;
        for (int count = 0; count < colliderRegions.Length; ++count) {
            if (colliderRegions[count].mReducedOutlineVertices != null && colliderRegions[count].mReducedOutlineVertices.Count != 0) {
                ++numActiveIslands;
            }
    ...
    bool SetEmptyMeshCollider() {
		...
		MeshCollider collider = this.TargetMeshColliderComponentNoReferencing;
		if (collider == null) {
            EnsureHasMeshColliderComponentsAndReferences(this.NumColliderAnimationFrames, mAnimatedColliderParameters.NumCollidersNeeded);
			collider = this.TargetMeshColliderComponentNoReferencing;
			if (collider == null) {
	...
	//-------------------------------------------------------------------------
    bool AddAlreadyGeneratedMeshColliderBackIn() {
        MeshCollider collider = this.TargetMeshColliderComponentNoReferencing;
        if (collider == null) {
            EnsureHasMeshColliderComponentsAndReferences(this.NumColliderAnimationFrames, mAnimatedColliderParameters.NumCollidersNeeded);
            collider = this.TargetMeshColliderComponentNoReferencing;
            if (collider == null) {
                MeshCollider[] addedColliderArray = AddEmptyMeshColliderComponents(1);
                if (addedColliderArray != null) {
                    collider = addedColliderArray[0];
                }
                if (collider == null) {
                    Debug.LogError("Error: Failed to add MeshCollider component!");
                ...
                mAnimatedColliderParameters.MeshCollidersToSwitch[mActiveColliderIndex] = collider;

                MeshCollider[] attachedColliders = TargetNodeToAttachMeshCollider.GetComponents<MeshCollider>();
                CorrectColliderOrder(attachedColliders, mAnimatedColliderParameters.MeshCollidersToSwitch);

                mAnimatedColliderParameters.RemovedFrameReferenceIndex[mActiveColliderIndex] = REMOVED_FRAME_REFERENCE_INDEX_NOTREMOVED;
            }
            LoadAlreadyGeneratedColliderToMeshCollider();
        }
        return true;
    }
	...
	bool LoadAlreadyGeneratedColliderToMeshCollider() {
        ...
        int finallyReferencedFrameIndex = mActiveColliderIndex;
        MeshCollider collider = this.MeshCollidersToSwitch(mActiveColliderIndex, out finallyReferencedFrameIndex); // with referencing
...
#if UNITY_4_3_AND_LATER
        if (mRegionIndependentParameters.TargetColliderType == TargetColliderType.PolygonCollider2D) {
			EnsureHasPolygonCollider2DComponentsAndReferences(numAnimationFrames, mAnimatedColliderParameters.NumCollidersNeeded);
		}
#if UNITY_SUPPORTS_EDGECOLLIDER2D
        else if (mRegionIndependentParameters.TargetColliderType == TargetColliderType.EdgeCollider2D) {
            EnsureHasEdgeCollider2DComponentsAndReferences(numAnimationFrames, mAnimatedColliderParameters.NumCollidersNeeded);
        }
#endif
        else if (mRegionIndependentParameters.TargetColliderType == TargetColliderType.MeshCollider) {
#endif
			EnsureHasMeshColliderComponentsAndReferences(numAnimationFrames, mAnimatedColliderParameters.NumCollidersNeeded);
#if UNITY_4_3_AND_LATER
		}
		else {
	...
	public void RemoveExistingPolygonCollider2DComponents() {
		...
		if (colliders != null) {
			for (int count = 0; count < colliders.Length; ++count) {
                UndoAware.DestroyImmediate(colliders[count]);
			}
		}
		mAnimatedColliderParameters.PolygonCollidersToSwitch = null;
	}
...
#if UNITY_SUPPORTS_EDGECOLLIDER2D
    //-------------------------------------------------------------------------
    public void RemoveExistingEdgeCollider2DComponents() {
        EdgeCollider2D[] colliders = TargetNodeToAttachMeshCollider.GetComponents<EdgeCollider2D>();
        if (colliders != null) {
            for (int count = 0; count < colliders.Length; ++count) {
                UndoAware.DestroyImmediate(colliders[count]);
            }
	...
	void CheckForSmoothMovesSpriteComponent(out bool hasSmoothMovesSpriteComponent, out Component smoothMovesSpriteComponent) {
        ...
        Component[] allComponents = this.gameObject.GetComponents<Component>();
        for (int componentIndex = 0; componentIndex < allComponents.Length; ++componentIndex) {
            Component currentComponent = allComponents[componentIndex];
	...
	static public bool CreateColliderPrefabsForAllOTContainerFrames(Component otSprite, AlphaMeshCollider referenceParameters, out List<GameObject> colliderPrefabAtTileIndex, out Vector2 atlasFrameSizeInPixels) {
        ...
        GameObject collidersNode = UndoAware.CreateGameObject(groupNodeName);
	...
	static public GameObject CreateColliderPrefabForOTContainerFrame(AlphaMeshCollider referenceParameters, out int tileIndex, out Vector2 resultFrameSize, Transform parentNode, object otContainerFrame, Texture2D texture) {
		...
		if (referenceParameters) {
            ...
            alphaMeshColliderComponent.mRegionIndependentParameters.IsTrigger = referenceParameters.mRegionIndependentParameters.IsTrigger;
	...
	static GameObject CreateAlphaMeshColliderGameObject(Transform parentNode, string gameObjectName, string customRegionName, Texture2D texture, Vector2 framePositionInPixels, Vector2 frameSizeInPixels, float frameRotation) {
        ...
        GameObject colliderNode = UndoAware.CreateGameObject(gameObjectName);
        ...
        AlphaMeshCollider alphaMeshCollider = UndoAware.AddComponent<AlphaMeshCollider>(colliderNode);
	...
	void InitTextureParams() {
		...
		mRegionIndependentParameters.NumColliderAnimationFrames = 1;
	...
	void ReadNormalImageParametersFromComponents() {
#if UNITY_4_3_AND_LATER
		if (mHasUnity43SpriteRendererComponent) {
			mIsAtlasUsed = UnityBuiltInSpriteTools.ReadUnity43SpriteParams(mUnity43SpriteRendererComponent,  out mMainTex, out mAtlasFrameTitle, out mAtlasFrameIndex, out mAtlasFramePositionInPixels, out mAtlasFrameSizeInPixels, out mAtlasFrameRotation, out mOutlineScale, out mOutlineOffset);
		}
		if (mHasUnity43SpriteAnimatorComponent) {
        ...
        if (mHasUGUIImageSpriteComponent) {
            mIsAtlasUsed = UnityBuiltInSpriteTools.ReadUGUIImageSpriteParams(mUGUIImageComponent, out mMainTex, out mAtlasFrameTitle, out mAtlasFrameIndex, out mAtlasFramePositionInPixels, out mAtlasFrameSizeInPixels, out mAtlasFrameRotation, out mOutlineScale, out mOutlineOffset);
        }
        if (mHasUGUIRawImageComponent)
        {
            mIsAtlasUsed = UnityBuiltInSpriteTools.ReadUGUIRawImageParams(mUGUIRawImageComponent, out mMainTex, out mAtlasFrameTitle, out mAtlasFrameIndex, out mAtlasFramePositionInPixels, out mAtlasFrameSizeInPixels, out mAtlasFrameRotation, out mOutlineScale, out mOutlineOffset);
        }
#endif
	...
	void EnsureSmoothMovesBoneAnimHasRestoreComponent(Component smoothMovesBoneAnimation) {
		...
		if (restoreComponent == null) {
            UndoAware.AddComponent<AlphaMeshColliderSmoothMovesRestore>(smoothMovesBoneAnimation.gameObject);
		}
	}
	...
	void EnsureHasUnity43AnimatedColliderComponents() {
		...
		if (switchComponent == null) {
            switchComponent = UndoAware.AddComponent<RuntimeAnimatedColliderSwitch>(this.gameObject);
			if (switchComponent == null) {
		...
		int numCollidersNeeded = mAnimatedColliderParameters.NumCollidersNeeded;
        int numAnimationFrames = this.NumColliderAnimationFrames;
		...
		if (mRegionIndependentParameters.TargetColliderType == TargetColliderType.MeshCollider) {
			...
			if (mAnimatedColliderParameters.MeshCollidersToSwitch == null || mAnimatedColliderParameters.MeshCollidersToSwitch.Length != numCollidersNeeded) {
				EnsureHasMeshColliderComponentsAndReferences(numAnimationFrames, numCollidersNeeded);
			}
		}
#if UNITY_SUPPORTS_EDGECOLLIDER2D
        else if (mRegionIndependentParameters.TargetColliderType == TargetColliderType.EdgeCollider2D) {
            mAnimatedColliderParameters.MeshCollidersToSwitch = null;
            mAnimatedColliderParameters.PolygonCollidersToSwitch = null;
            if (mAnimatedColliderParameters.EdgeCollidersToSwitch == null || mAnimatedColliderParameters.EdgeCollidersToSwitch.Length != numCollidersNeeded) {
                EnsureHasEdgeCollider2DComponentsAndReferences(numAnimationFrames, numCollidersNeeded);
            }
        }
#endif
            ...
            if (mAnimatedColliderParameters.PolygonCollidersToSwitch == null || mAnimatedColliderParameters.PolygonCollidersToSwitch.Length != numCollidersNeeded) {
				EnsureHasPolygonCollider2DComponentsAndReferences(numAnimationFrames, numCollidersNeeded);
			}
		}
#if UNITY_SUPPORTS_EDGECOLLIDER2D
        switchComponent.EdgeCollidersToSwitchNoReferencing = mAnimatedColliderParameters.EdgeCollidersToSwitch;
#endif
        switchComponent.PolygonCollidersToSwitchNoReferencing = mAnimatedColliderParameters.PolygonCollidersToSwitch;
		switchComponent.MeshCollidersToSwitchNoReferencing = mAnimatedColliderParameters.MeshCollidersToSwitch;
        switchComponent.RemovedFrameReferenceIndex = mAnimatedColliderParameters.RemovedFrameReferenceIndex;
    }

	//-------------------------------------------------------------------------
	void EnsureDataIsPreparedForMultipleColliders() {
		int oldSize = mGeneratedColliderData.Length;
		int desiredSize = mRegionIndependentParameters.NumColliderAnimationFrames;
	...
	void EnsureOTTilesSpriteHasUpdateComponent(Component otTilesSprite) {
		...
		if (updateComponent == null) {
            updateComponent = UndoAware.AddComponent<AlphaMeshColliderUpdateOTTilesSpriteColliders>(otTilesSprite.gameObject);
    ...
    //-------------------------------------------------------------------------
    void CheckForUGUIImageSpriteComponent(out bool hasUGUIImageSpriteComponent, out UnityEngine.UI.Image uguiImageComponent) {
        uguiImageComponent = this.gameObject.GetComponent<UnityEngine.UI.Image>();
        hasUGUIImageSpriteComponent = (uguiImageComponent != null);
    }

    //-------------------------------------------------------------------------
    void CheckForUGUIRawImageComponent(out bool hasUGUIRawImageComponent, out UnityEngine.UI.RawImage uguiImageComponent)
    {
        uguiImageComponent = this.gameObject.GetComponent<UnityEngine.UI.RawImage>();
        hasUGUIRawImageComponent = (uguiImageComponent != null);
    }
    ...
    bool ReadUnity43SpriteAnimatorParams(UnityEngine.Animator unity43Animator, ref AnimatedColliderParameters animatedColliderParams, ref RegionIndependentParameters regionIndependentParams) {
        ...
        string[] spriteIDStrings = null;
        int numFrames = 0;
        bool wasSuccessful = UnityBuiltInSpriteTools.ReadUnity43SpriteAnimatorParams(this.gameObject, unity43Animator, ref spriteFrames, ref spriteIDStrings, ref numFrames);
        animatedColliderParams.SpriteFrames = spriteFrames;
        animatedColliderParams.ColliderIDStrings = spriteIDStrings;
        regionIndependentParams.NumColliderAnimationFrames = numFrames;
	...
	bool GenerateUnreducedColliderMesh(ref GeneratedColliderData generatedColliderData) {
        ...
        outlineAlgorithm.LowerTop = mRegionIndependentParameters.LowerTop;
        outlineAlgorithm.RaiseBottom = mRegionIndependentParameters.RaiseBottom;
        outlineAlgorithm.CutLeft = mRegionIndependentParameters.CutLeft;
        outlineAlgorithm.CutRight = mRegionIndependentParameters.CutRight;
        outlineAlgorithm.ExpandTop = mRegionIndependentParameters.ExpandTop;
        outlineAlgorithm.ExpandBottom = mRegionIndependentParameters.ExpandBottom;
        outlineAlgorithm.ExpandLeft = mRegionIndependentParameters.ExpandLeft;
        outlineAlgorithm.ExpandRight = mRegionIndependentParameters.ExpandRight;

        bool wasSuccessful = outlineAlgorithm.BinaryAlphaThresholdImageFromTexture(ref generatedColliderData.mBinaryImage, out generatedColliderData.mBinaryImageWidth, out generatedColliderData.mBinaryImageHeight,
                                                                                   UsedTexture, mRegionIndependentParameters.AlphaOpaqueThreshold,
        ...
        int imageSize = generatedColliderData.mBinaryImageWidth * generatedColliderData.mBinaryImageHeight;
        if (generatedColliderData.mClassificationImage == null || generatedColliderData.mClassificationImage.Length < imageSize) {
            generatedColliderData.mClassificationImage = new int[imageSize];
        }
        
        /*bool anyIslandsFound =*/ CalculateIslandStartingPoints(generatedColliderData.mBinaryImage, generatedColliderData.mBinaryImageWidth, generatedColliderData.mBinaryImageHeight, ref generatedColliderData.mClassificationImage, ref generatedColliderData.mIslandsWorkList, ref generatedColliderData.mSeaRegionsWorkList);
        /*if (!anyIslandsFound) { // we now tolerate if there is no island pixel, one could still enable the single sea-region.
            return false;
        }*/
		
		SetupColliderRegions(out generatedColliderData.mColliderRegions, generatedColliderData.mIslandsWorkList, generatedColliderData.mSeaRegionsWorkList);

		CopyOldColliderRegionParametersForBackwardsCompatibility();
		SetupColliderRegionParameters(ref mIslandRegionParameters, ref mSeaRegionParameters, mRegionIndependentParameters.DefaultMaxPointCount, generatedColliderData.mIslandsWorkList, generatedColliderData.mSeaRegionsWorkList);
		...
		CalculateUnreducedOutlineForAllColliderRegions(ref generatedColliderData.mColliderRegions, ref outlineAlgorithm, mRegionIndependentParameters, mIslandRegionParameters, mSeaRegionParameters, generatedColliderData.mBinaryImage, generatedColliderData.mBinaryImageWidth, generatedColliderData.mBinaryImageHeight, ccwVertexOrder);
        return true;
	}
	...
	bool ReduceAndStoreColliderMesh(ref GeneratedColliderData generatedColliderData) {
		...
		for (int count = 0; count < colliderRegions.Length; ++count) {
			...
			if (needsRegionReduction) {
				outlineAlgorithm.ReduceOutline(region.mIntermediateOutlineVertices, ref region.mReducedOutlineVertices, ref region.mUsedIndices, region.mOutlineVertexOrderIsCCW);
			}
	...
	//-------------------------------------------------------------------------
	public static void SetupColliderRegions(out ColliderRegionData[] colliderRegions, List<IslandDetector.Region> islands, List<IslandDetector.Region> seaRegions) {
		
		int numColliderRegions = islands.Count + seaRegions.Count;
	...
	public static void SetupColliderRegionParameters(ref ColliderRegionParameters[] islandRegionParameters, ref ColliderRegionParameters[] seaRegionParameters,
													 ...
													 List<IslandDetector.Region> islands, List<IslandDetector.Region> seaRegions) {

		// Islands
		int oldIslandRegionParametersLength = 0;
		if (islandRegionParameters == null) {			
			islandRegionParameters = new ColliderRegionParameters[islands.Count];
		}
		else {
			oldIslandRegionParametersLength = islandRegionParameters.Length;
			if (islands.Count > oldIslandRegionParametersLength) {
				System.Array.Resize(ref islandRegionParameters, islands.Count);
			}
		}
		...
		for (int newIndex = oldIslandRegionParametersLength; newIndex < islands.Count; ++newIndex) {
		...
		if (seaRegionParameters == null) {			
			seaRegionParameters = new ColliderRegionParameters[seaRegions.Count];
		}
		else {
			oldSeaRegionParametersLength = seaRegionParameters.Length;
			if (seaRegions.Count > oldSeaRegionParametersLength) {
				System.Array.Resize(ref seaRegionParameters, seaRegions.Count);
			}
		}
		// set RegionUpdateCalculationNeeded at old entries
		...
		// add new ColliderRegionParameters at new ones
		for (int seaRegionIndex = oldSeaRegionParametersLength; seaRegionIndex < seaRegions.Count; ++seaRegionIndex) {
	...
	public void CopyColliderRegionsForBackwardsCompatibility() {
        ...
        if (mBinaryImage != null) {
            width = this.mBinaryImage.GetLength(1);
            height = this.mBinaryImage.GetLength(0);
            
        }
        mGeneratedColliderData[0].mBinaryImageWidth = width;
        mGeneratedColliderData[0].mBinaryImageHeight = height;

        //mGeneratedColliderData[0].mBinaryImage = new bool[width * height];
        //System.Buffer.BlockCopy(this.mBinaryImage, 0, mGeneratedColliderData[0].mBinaryImage, 0, width * height);
	...
	public static void CalculateUnreducedOutlineForAllColliderRegions(ref ColliderRegionData[] colliderRegions, ref PolygonOutlineFromImageFrontend outlineAlgorithm,
		...
		for (int count = 0; count < colliderRegions.Length; ++count) {
			...
			if (regionParameters.EnableRegion) {
	            ...
	            outlineAlgorithm.UnreducedOutlineFromBinaryImage(ref colliderRegions[count].mIntermediateOutlineVertices, binaryImage, binaryImageWidth, binaryImageHeight, colliderRegions[count].mDetectedRegion.mPointAtBorder, colliderRegions[count].mRegionIsIsland, outputInNormalizedSpace, regionVertexOrder);
			}
			else {
