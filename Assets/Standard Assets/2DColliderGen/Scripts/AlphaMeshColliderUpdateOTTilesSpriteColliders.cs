...
public class AlphaMeshColliderUpdateOTTilesSpriteColliders : MonoBehaviour {
	...
	void DeleteAndCreateAllTileColliderPrefabs() {
		...
		if (groupNode != null) {
            UndoAware.DestroyImmediate(groupNode);
		}
	...
	private bool InstantiateAllTileColliders() {
		...
		mTileObjectsGroupNode = new GameObject(RUNTIME_GROUP_NODE_NAME); // called at runtime only
...
#if UNITY_EDITOR
		if (Application.isEditor && !Application.isPlaying) {
            UndoAware.DestroyImmediate(gameObject);
		}
		else {
#endif
