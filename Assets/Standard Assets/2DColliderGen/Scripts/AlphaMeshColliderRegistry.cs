...
public class AlphaMeshColliderRegistry : MonoBehaviour {
	...
	public class ColliderGroup {
        ...
        public AlphaMeshCollider.AnimatedColliderParameters mAnimatedColliderParameters = null;
        ...
        get {
            if (mInstance == null) {
				...
				if (existingSingleton != null) {
                    ...
                    GameObject gameObject = UndoAware.CreateGameObject("AlphaMeshColliderRegistry");
                    mInstance = UndoAware.AddComponent<AlphaMeshColliderRegistry>(gameObject);
				}
            }
            return mInstance; 
        }
    ...
    //-------------------------------------------------------------------------
    public void UpdateColliderComponentValues(AlphaMeshCollider target) {
    ...
    //-------------------------------------------------------------------------
    public void DecreaseActiveFrameReferenceFrameIndex(AlphaMeshCollider target) {
        target.DecreaseActiveFrameReferenceFrameIndex();
        UpdateSimilarCollidersAndGroupToTarget(target);
    }
	...
	protected void AssignValuesFromInstanceToGroup(AlphaMeshCollider target, ColliderGroup colliderGroup) {
        ...
        colliderGroup.mAnimatedColliderParameters = target.AnimatedColliderParams;
	...
	protected bool IsColliderGroupValid(ColliderGroup colliderGroup) {
		...
		if (colliderGroup.mIslandRegionParameters == null ||
            ...
            (colliderGroup.mRegionIndependentParameters.TargetColliderType == AlphaMeshCollider.TargetColliderType.MeshCollider && colliderGroup.mColliderMeshes == null)) {
			...
			if (colliderGroup.mRegionIndependentParameters.TargetColliderType == AlphaMeshCollider.TargetColliderType.MeshCollider) {
            ...
            else if (colliderGroup.mRegionIndependentParameters.TargetColliderType == AlphaMeshCollider.TargetColliderType.EdgeCollider2D) {
                // EdgeCollider2D
                bool allEdgeCollidersAreNull = true;
                for (int index = 0; index < colliderGroup.mGeneratedColliderData.Length; ++index) {
                    if (colliderGroup.mGeneratedColliderData[index] != null &&
                        ...
                        allEdgeCollidersAreNull = false;
                }
                if (allEdgeCollidersAreNull) {
                    return false;
                }
            }
#endif
