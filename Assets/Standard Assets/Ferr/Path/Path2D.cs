...
namespace Ferr {
	...
	public class Path2D : ISVGElement {
		...
		public float GetAngle(int aRawIndex) {
			Vector2 curr = _points[aRawIndex];
			return Vector2.Angle(
				_points[PathUtil.WrapIndex(aRawIndex-1, _points.Count, _closed)] - curr,
				_points[PathUtil.WrapIndex(aRawIndex+1, _points.Count, _closed)] - curr);
		}
		public float GetSmoothedAngle(int aRawIndex) {
			if (_points.Count < 2) return 180;

			if (!_closed && (aRawIndex == 0 || aRawIndex == _points.Count-1))
				return 180;
			
			int currId = PathUtil.WrapIndex(aRawIndex,   _points.Count, _closed);
            Vector2 curr = _points[currId];
			if (_pointControls[currId].type == PointType.Free) {
				return PathUtil.ClockwiseAngle(_pointControls[currId].controlPrev.normalized, _pointControls[currId].controlNext.normalized);
			}

			int prevId = PathUtil.WrapIndex(aRawIndex-1, _points.Count, _closed);
            Vector2 prev = _points[prevId] - curr;
			if (_pointControls[prevId].type != PointType.Sharp)
				prev += _pointControls[prevId].controlNext;

			int nextId = PathUtil.WrapIndex(aRawIndex+1, _points.Count, _closed);
            Vector2 next = _points[nextId] - curr;
			if (_pointControls[nextId].type != PointType.Sharp)
				next += _pointControls[nextId].controlPrev;

            prev.Normalize();
            next.Normalize();

            return PathUtil.ClockwiseAngle(prev, next);
		}
		public Vector2 GetSmoothedNormal(int aRawIndex) {
			if (_points.Count < 2) return Vector2.up;
			
			int currId = PathUtil.WrapIndex(aRawIndex,   _points.Count, _closed);
            Vector2 curr = _points[currId];
			if (_pointControls[currId].type == PointType.Free) {
				Vector2 np = _pointControls[currId].controlPrev.normalized;
				Vector2 nn = _pointControls[currId].controlNext.normalized;
				return (new Vector2(-np.y, np.x) + new Vector2(nn.y, -nn.x)).normalized;
			}
			...
			if (!_closed && (aRawIndex == 0 || aRawIndex == _points.Count-1)) {
				...
				} else if (aRawIndex == _points.Count-1) {
					return new Vector2(-prev.y, prev.x);
				}
			}

            return (new Vector2(-prev.y, prev.x) + new Vector2(next.y, -next.x)).normalized;
		}
